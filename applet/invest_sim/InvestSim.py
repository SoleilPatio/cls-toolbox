...
class LoggingHandler(logging.StreamHandler):
    ...
    def emit(self, record):
        ...
        if( self.gui_statusbar_obj and record.levelno >= logging.WARNING):
...
class InvestSimFrame(MyFrame):
    ...
    def ClearStatusMessage(self, index=2):
        self.GUI_statusbar.SetStatusText("", i=index)


    
    def OnCloudLoad(self, event):  # wxGlade: MyFrame.<event_handler>
        self.ClearStatusMessage() #clear status message first

        self.GUI_statusbar.SetStatusText(u"讀取雲端資料中...", i=0)
        self.Control.Initial(CONFIG_FILE)
        self.Control.LoadData()
        self.Control.Policy()
        self.GUI_statusbar.SetStatusText(u"雲端資料讀取完畢!", i=0)


    def OnInputData(self, event):  # wxGlade: MyFrame.<event_handler>
        self.ClearStatusMessage() #clear status message first
        import webbrowser
        self.GUI_statusbar.SetStatusText(u"開啓Google試算表輸入資料", i=0)
    ...
    def OnQuickSimProductSelect(self, event):  # wxGlade: MyFrame.<event_handler>
        self.ClearStatusMessage() #clear status message first
        self.GUI_statusbar.SetStatusText(u"切換產品:%s"%event.String, i=0)
        self.ViewQuickSim.RestoreGuiFromCacheByProduct(self, event.String)
    ...
    def OnQuickSimApply(self, event):  # wxGlade: MyFrame.<event_handler>
        self.ClearStatusMessage() #clear status message first
        self.ViewQuickSim.Render(   
            self,                           #Frame object
            self.matplotlib_figure_2,       #Figure (Canvas of matplotlib)
        ...
        self.GUI_statusbar.SetStatusText(u"試算完成", i=0)
